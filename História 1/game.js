...
function showMsg(text, type='info'){
  ...
  div.innerHTML = type==='ok' ? `⭐ ${text}` : (type==='err' ? `⚠️ ${text}` : `💡 ${text}`);
  messages.appendChild(div);
  // fala apenas para ok/err e algumas infos chaves
  if (type === 'ok' || type === 'err' || /começar|dica/i.test(text)) speak(text);
}

function checkWin(){
  if(state.correct === state.total){
    setTimeout(()=>{ win.style.display='grid'; confettiBurst(); speak('Parabéns! Tudo conectado.'); }, 250);
  }
}
...
function confettiBurst(){
  ...
  for(let i=0;i<count;i++){
    ...
    s.animate([
      ...
      { transform:`translate(${dx}px, ${dy}px) scale(${Math.random()*1+0.6})`, opacity:0 }
...
function playChime(ok=true){
  try{
    ...
    o.frequency.exponentialRampToValueAtTime(ok? 1200 : 140, audioCtx.currentTime + 0.18);
...
slots.forEach(slot=>{
  slot.addEventListener('dragover', e=>{ e.preventDefault(); e.dataTransfer.dropEffect = 'move'; slot.classList.add('highlight'); });
  ...
  slot.addEventListener('drop', e=>{
    ...
    if(!item) return;
    if(slot.classList.contains('correct')){ showMsg('Este espaço já está ocupado. Tente outro 😊', 'err'); playChime(false); return; }
    if(slot.dataset.slot === device){
      ...
      playChime(true);
      pop(item);
      state.correct++; checkWin();
    } else {
      tray.appendChild(item);
      wiggle(item); playChime(false);
      showMsg('Ops! Esse não é o lugar. Tente de novo!', 'err');
    }
  });
});

function wiggle(el){ el.animate([{ transform:'translateX(0)' },{ transform:'translateX(-8px)' },{ transform:'translateX(8px)' },{ transform:'translateX(0)' }], { duration:280, easing:'ease-in-out' }); }
function pop(el){ el.animate([{ transform:'scale(1)' },{ transform:'scale(1.06)' },{ transform:'scale(1)' }], { duration:220, easing:'ease-out' }); }
function capitalize(s){ return s.charAt(0).toUpperCase() + s.slice(1); }

resetBtn.addEventListener('click', resetGame);
function resetGame(){
  win.style.display='none'; messages.innerHTML=''; state.correct = 0; slots.forEach(s=> s.classList.remove('correct'));
  items.forEach(it=>{ tray.appendChild(it); it.classList.remove('locked'); it.setAttribute('draggable','true'); });
  showMsg('Vamos começar! Combine as figuras com os lugares certos. Dica: Teclado e Mouse usam USB.', 'info');
}
